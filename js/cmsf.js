import {
  ...
  window.add_cmsf_card = async () => {
    // alert("hello");
    var reference = push(ref(db, "Cmsf/"));
    ...
    close.click("");
    swal({
      title: "CMSF Member Add Succesfully !",
      icon: "success",
    });
    name.value = " ";
    surname.value = " ";
  ...
  window.updatecard = async () => {
    ...
    if (editimage.files.length == 0) {
      ...
      var reference = ref(db, `Cmsf/${edituid}`);
